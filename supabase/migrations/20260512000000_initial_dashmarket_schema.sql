...
create table public.marketplace_account_credentials (
...
create table public.contribution_margin_snapshots (
  id uuid primary key default gen_random_uuid(),
  organization_id uuid not null references public.organizations(id) on delete cascade,
  product_id uuid not null references public.products(id) on delete cascade,
...
create index marketplace_accounts_organization_idx on public.marketplace_accounts (organization_id);
create index products_organization_idx on public.products (organization_id);
create index sku_costs_product_validity_idx on public.sku_costs (product_id, valid_from, valid_to);
create index marketplace_listings_product_idx on public.marketplace_listings (product_id);
create index orders_sold_at_idx on public.orders (organization_id, sold_at desc);
create index order_items_sku_idx on public.order_items (organization_id, seller_sku);
create index inventory_snapshots_lookup_idx on public.inventory_snapshots (organization_id, seller_sku, captured_at desc);
create index advertising_metrics_date_idx on public.advertising_metrics (organization_id, metric_date desc);
create index contribution_margin_period_idx on public.contribution_margin_snapshots (organization_id, period_start, period_end);
create index sync_runs_status_idx on public.sync_runs (organization_id, status, started_at desc);
create index marketplace_events_lookup_idx on public.marketplace_events (provider, external_user_id, topic, received_at desc);
...
create trigger marketplace_account_credentials_set_updated_at
before update on public.marketplace_account_credentials
for each row execute function public.set_updated_at();
...
create trigger sku_costs_set_updated_at
...
create policy "organization_members_insert_member"
...
create policy "organization_members_delete_member"
on public.organization_members for delete
using (public.org_member_role(organization_id) in ('owner', 'admin'));

create policy "marketplace_accounts_org_member"
on public.marketplace_accounts for all
using (public.is_org_member(organization_id))
with check (public.is_org_member(organization_id));

create policy "products_org_member"
on public.products for all
using (public.is_org_member(organization_id))
with check (public.is_org_member(organization_id));

create policy "sku_costs_org_member"
on public.sku_costs for all
using (public.is_org_member(organization_id))
with check (public.is_org_member(organization_id));

create policy "marketplace_listings_org_member"
on public.marketplace_listings for all
using (public.is_org_member(organization_id))
with check (public.is_org_member(organization_id));

create policy "orders_org_member"
on public.orders for all
using (public.is_org_member(organization_id))
with check (public.is_org_member(organization_id));

create policy "order_items_org_member"
on public.order_items for all
using (public.is_org_member(organization_id))
with check (public.is_org_member(organization_id));

create policy "inventory_snapshots_org_member"
