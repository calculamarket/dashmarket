...
const SKU_ATTRIBUTE_IDS = ["SELLER_SKU", "SKU", "ITEM_SKU"];

function extractSellerSku(item: MercadoLivreItem) {
  const itemSku =
    normalizeSku(item.seller_custom_field) ??
    getAttributeValue(item.attributes, SKU_ATTRIBUTE_IDS);
  ...
  for (const variation of item.variations ?? []) {
    const variationSku =
      ...
      getAttributeValue(variation.attributes, SKU_ATTRIBUTE_IDS) ??
      getAttributeValue(variation.attribute_combinations, SKU_ATTRIBUTE_IDS);

    if (variationSku) return variationSku;
  }

  // Fallback to item ID if no SKU-like field is found
  return item.id;
}
