...
```bash
https://dashmarketml.vercel.app/api/whatsapp/webhook?token=SEU_SEGREDO
```
...
Use `webhookByEvents` como `false` quando configurar a instancia. Se a sua Evolution estiver com `webhookByEvents` ligado, ela pode chamar uma URL como `/api/whatsapp/webhook/messages-upsert`; o DASHMARKET tambem aceita esse formato, mas a configuracao sem URL por evento e mais simples.
